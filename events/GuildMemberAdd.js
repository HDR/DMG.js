...
client.on('guildMemberAdd', async (member ) => {
    ...
    if(Math.trunc(Math.ceil(currentDate.getTime() - member.user.createdAt.getTime()) / (1000 * 3600 * 24)) < 28 && !member.user.bot) {
        member.kick('Account is too new (less than 28 days old)')
    }
});
